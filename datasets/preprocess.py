...
def resize_volume_new(path, center=None, output_shape=(192, 192, 192), tumor=False, re_ori=True):
    img = read_nifti_file(path, tumor=tumor, re_ori=re_ori)
    img_shape = img.shape
    output_shape = list(output_shape)

    if center is None or (np.array(center) == None).any():
        center = [s // 2 for s in img_shape]

    half_shape = [s // 2 for s in output_shape]

    # Compute raw start/end (could be out-of-bounds)
    start = [center[i] - half_shape[i] for i in range(3)]
    end = [start[i] + output_shape[i] for i in range(3)]

    # Compute padding if start < 0 or end > image shape
    pad_before = [max(0, -start[i]) for i in range(3)]
    pad_after = [max(0, end[i] - img_shape[i]) for i in range(3)]

    # Clip start/end to be within image bounds
    clipped_start = [max(0, start[i]) for i in range(3)]
    clipped_end = [min(img_shape[i], end[i]) for i in range(3)]
...
def process_scan(path, mask = False, resize = True,norm_method = 'mm',output_shape=(192,192,192),center=None,re_ori = True,LBPA=False):
...
#     volume = read_nifti_file(path)
    if resize == True:
        ...
        if LBPA==True:
            volume = resize_volume_LBPA(path,output_shape =output_shape,center=center,re_ori = re_ori)
    else:
        volume = read_nifti_file(path)
...
def process_scan_new(path, mask = False, resize = True,norm_method = 'mm',output_shape=(228,228,228),center=None,re_ori = True):
    """Read and resize volume"""
    # Read scan
#     volume = read_nifti_file(path)
    if resize == True:
        volume = resize_volume_new(path,output_shape =output_shape,center=center,re_ori = re_ori)
    else:
        volume = read_nifti_file(path)
